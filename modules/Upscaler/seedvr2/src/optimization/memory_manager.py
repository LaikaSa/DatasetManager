...
# (wrapped so a non-UTF-8 console, e.g. Windows cp1252, can't crash the import)
try:
    vram_info = get_basic_vram_info(device=None)
    if "error" not in vram_info:
        backend = "MPS" if is_mps_available() else "CUDA"
        print(f"[SeedVR2] Initial {backend} memory: {vram_info['free_gb']:.2f}GB free / {vram_info['total_gb']:.2f}GB total")
    else:
        print(f"[SeedVR2] Memory check: {vram_info['error']} - No available backend!")
except Exception:
    pass
